...
class JwtAccessToken extends AccessToken
{
    ...
    public function __construct(<PublicKeyInterface> publicKeyStorage = null, <AccessTokenStorageInterface> tokenStorage = null, <RefreshTokenInterface> refreshStorage = null, array config = [], <EncryptionInterface> encryptionUtil = null)
    {
        ...
        let config = array_merge(["store_encrypted_token_string": true, "issuer": ""], config);
        if is_null(tokenStorage) {
            // a pass-thru, so we can call the parent constructor
            let tokenStorage = new Memory();
        }
    ...
    {
        // payload to encrypt
        var payload, access_token, token_to_store, token;
        let payload = this->createPayload(client_id, user_id, scope);

        /*
         * Encode the payload data into a single JWT access_token string
         */
        let access_token = this->encodeToken(payload, client_id);
         ...
         */
        let token_to_store = this->config["store_encrypted_token_string"] ? access_token : payload["id"];
        ...
        if includeRefreshToken && this->refreshStorage {
            var refresh_token, expires = 0;
            let refresh_token = this->generateRefreshToken();
            if this->config["refresh_token_lifetime"] > 0 {
                let expires = time() + this->config["refresh_token_lifetime"];
            }
    ...
    {
        var private_key, algorithm;
        let private_key = this->publicKeyStorage->getPrivateKey(client_id);
    ...
    protected function createPayload(client_id, user_id, scope = null)
    {
        // token to encrypt
        var expires, id;
        let expires = time() + this->config["access_lifetime"];
        let id = this->generateAccessToken();

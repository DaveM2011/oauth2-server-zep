...
class Bearer implements TokenTypeInterface
{
     ...
     */
    public function requestHasToken(<RequestInterface> request) -> boolean
    {
        var headers;
     ...
     *
     */
    public function getAccessTokenParameter(<RequestInterface> request, <ResponseInterface> response) -> string | null
    {
        var headers, methodsUsed, matches, contentType, pos;
        ...
        if request->request(this->config["token_param_name"]) {
            ...
            let pos = strpos(contentType, ";");
            if pos !== false {
                let contentType = substr(contentType, 0, pos);
            }

            if typeof contentType !== "null" && contentType != "application/x-www-form-urlencoded" {
